...
std::string ucharToBitString(uint16_t x)
{
    std::cout << "x is : " << x << " ";
    std::string s = "";
    for(int i = 0; i < 16; i++)
...
int Visitor::visitTree(ASTProgNode *prog, std::string fileName)
{
    ...
    while(child != nullptr){
        ...
        std::cout << "line : " << i << " is : " << ucharToBitString(currentInstruction) << std::endl;
...
{
    std::cout << "I instruction visited" << std::endl;
    uint16_t IR;
    IR = std::any_cast<uint8_t>(this->visitChildren(ctx->getChildren(0)));
    std::cout << ucharToBitString(IR) << std::endl;
    IR = IR <<4;
...
{
    std::cout << "J instruction visited" << std::endl;
    uint16_t IR;
...
{
    std::cout << "L instruction visited" << std::endl;
    uint16_t IR;
    IR = std::any_cast<uint8_t>(this->visitChildren(ctx->getChildren(0)));
    IR = IR <<4;
    IR += std::any_cast<uint8_t>(this->visitChildren(ctx->getChildren(1)));
